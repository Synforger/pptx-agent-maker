...
- `task lint` — stack の linter (= stub、 `Taskfile.local.yml` 等で埋める)
- `task test:unit` — `tests/` を pytest で実行 (= 再描画判定とフォルダ走査の絞り込み)
...
詳細は [`.tooling/local-ci/`](../../.tooling/local-ci/) 内のスクリプトを参照。
